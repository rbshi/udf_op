    ...
    with open(kernel_name + '.csv', newline='') as csvfile:
        ...
        for row in spamreader:
            ...
            for idx_kernel in range(num_kernel):
                cfgfile.write("sp=" + kernel_name + "_" + str(idx_kernel+1) + ".p_hbm_in:HBM[0:31]\n")
                cfgfile.write("sp=" + kernel_name + "_" + str(idx_kernel+1) + ".p_hbm_out:HBM[0:31]\n")
            cfgfile.write("nk=" + kernel_name + ":" + str(num_kernel) + "\n")
            if auto_alloc:
                slr_dense = float(row[4])
                n_slr = kernel_alloc(kernel_name, num_kernel, slr_dense, 2)
            ...
            print("[Info] Config-" + str(idx) + " dumped.")
