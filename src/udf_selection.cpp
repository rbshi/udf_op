...
void datamover_read(std::vector<int*> buffer_d_hbm_out,
  ...
  for (uint32_t p = 0; p < out->m_num_partitions; p++) {
    ...
    // cout << "positivies: " << buffer_d_hbm_out[p][0] << endl;
...
int main(int argc, char **argv) {
  ...
  if (argc != 8) {
    cout << "Usage: ./testbench <num_values> <lower> <upper> <selectivity> <num_kernel> <num_times> <.xclbin>" << endl;
    return 1;
  }
  ...
  float selectivity = atof(argv[4]);
  int num_kernel = atoi(argv[5]);
  int num_times = atoi(argv[6]);
  std::string xclbin_fnm = argv[7];
  srand(3);

  // if (selectivity!=0.0){
  lower = 0; upper = int(num_values * selectivity);
  ...
  in_column.populate_int_column(num_values, 'u', 's', 0xDEADBEEF);
  ...
  // prepare the result buffer (time consuming)
  ...
  std::chrono::duration<double> run_time_w_copyback(0);
...
#ifdef HBM_PARTITION
    for (int i = 0; i < num_kernel; i++){
      int size_copyback = int(float(in_column.m_num_lines[i] * BYTES_IN_HBM_LINE) * selectivity * RES_BUF_FACTOR);
      hbm_buffer[i].sync(XCL_BO_SYNC_BO_FROM_DEVICE, (size_copyback > hbm_size? hbm_size : size_copyback), 0);
    }
#else
    // hbm_buffer.clear(); hbm_buffer.resize(num_kernel);
    for (int i = 0; i < num_kernel * 1; i++) {
  ...
  auto kernel_copyback_end = std::chrono::high_resolution_clock::now();
  run_time_w_copyback = std::chrono::duration<double>(kernel_copyback_end - kernel_start);
  ...
  double run_time_w_copyback_in_sec = run_time_w_copyback.count();
  // kernel_time_in_sec /= num_kernel;

  double result = (float)num_values * num_times * sizeof(uint32_t);
  result /= 1024;               // to KB
  result /= 1024;               // to MB
  result /= 1024;               // to GB
  result /= kernel_time_in_sec; // to GBps

  std::cout << "Kernel time = " << kernel_time_in_sec << " sec" << std::endl;
  std::cout << "Run time (with copyback) = " << run_time_w_copyback_in_sec << " sec" << std::endl;
