...
#include "../../../../sw/ml/models.hpp"

#define HBM_SIZE 32768*512

void datamover_write(
	hbm_gmem_t hbm_gmem,
	dataset* ds)
{
	// Transfer samples
	for (unsigned i = 0; i < ds->m_num_samples; i++) {
		for (unsigned j = 0; j < ds->m_num_features_in_lines; j++) {
			for (uint32_t k = 0; k < FLOATS_IN_HBM_LINE; k++) {
				uint32_t* temp = (uint32_t*)&(ds->m_row_samples[i*ds->m_num_features_in_lines*FLOATS_IN_LINE + j*FLOATS_IN_LINE + k]);
				hbm_gmem.p1[ds->m_samples_hbm_offset + i*ds->m_num_features_in_lines + j](BITS_IN_FLOAT*(k+1)-1, BITS_IN_FLOAT*k) = *temp;
			}
			for (uint32_t k = 0; k < FLOATS_IN_HBM_LINE; k++) {
				uint32_t* temp = (uint32_t*)&(ds->m_row_samples[i*ds->m_num_features_in_lines*FLOATS_IN_LINE + j*FLOATS_IN_LINE + FLOATS_IN_HBM_LINE + k]);
				hbm_gmem.p2[hbm_gmem.strided_hbm_offset + ds->m_samples_hbm_offset + i*ds->m_num_features_in_lines + j](BITS_IN_FLOAT*(k+1)-1, BITS_IN_FLOAT*k) = *temp;
			}
		}
	}
	...
	for (unsigned i = 0; i < ds->m_num_samples_in_lines; i++) {
		for (uint32_t k = 0; k < FLOATS_IN_HBM_LINE; k++) {
			...
			hbm_gmem.p1[ds->m_labels_hbm_offset + i](BITS_IN_FLOAT*(k+1)-1, BITS_IN_FLOAT*k) = *temp;
		}
		for (uint32_t k = 0; k < FLOATS_IN_HBM_LINE; k++) {
			uint32_t* temp = (uint32_t*)&(ds->m_labels[i*FLOATS_IN_LINE + FLOATS_IN_HBM_LINE + k]);
			hbm_gmem.p2[hbm_gmem.strided_hbm_offset + ds->m_labels_hbm_offset + i](BITS_IN_FLOAT*(k+1)-1, BITS_IN_FLOAT*k) = *temp;
		}
	}
}
	...
	models* ms)
{
	for (unsigned e = 0; e < ms->m_x_num_epochs; e++) {
		for (unsigned j = 0; j < ms->m_x_in_lines; j++) {
			for (uint32_t k = 0; k < FLOATS_IN_HBM_LINE; k++) {
				uint32_t temp = hbm_gmem.p1[ms->m_x_hbm_offset + e*ms->m_x_in_lines + j](BITS_IN_FLOAT*(k+1)-1, BITS_IN_FLOAT*k);
				ms->m_x_hbm[e*ms->m_x_in_lines*FLOATS_IN_LINE + j*FLOATS_IN_LINE + k] = *((float*)(&temp));
					
			}
			for (uint32_t k = 0; k < FLOATS_IN_HBM_LINE; k++) {
				uint32_t temp = hbm_gmem.p2[hbm_gmem.strided_hbm_offset + ms->m_x_hbm_offset + e*ms->m_x_in_lines + j](BITS_IN_FLOAT*(k+1)-1, BITS_IN_FLOAT*k);
				ms->m_x_hbm[e*ms->m_x_in_lines*FLOATS_IN_LINE + j*FLOATS_IN_LINE + FLOATS_IN_HBM_LINE + k] = *((float*)(&temp));
			}
		}
	}
}
...
int main(int argc, char *argv[]) {
	...
	models models_inst(&dataset_inst);
	...
	datamover_write(hbm_gmem, &dataset_inst);
	...
	sgd_main(
		hbm_gmem.p1, hbm_gmem.p2, hbm_gmem.strided_hbm_offset,
		samples_addr,
		labels_addr,
		model_addr,
	...
	datamover_read(hbm_gmem, &models_inst);
